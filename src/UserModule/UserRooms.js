import { useGetroombookingsQuery } from "../rtkQuery";
...
const UserRooms = () => {
    ...
    const navigateToAddBookingRoom = (booking) => {
        navigate('/userbooking/:id',{state:{booking}})
    }
            ...
            {data?.map((room) => (
                <div className="card m-5 p-4" key={room.id}>
                    <div className="row">
                        ...
                        <div className="col-6">
                            <p style={{ color: "blue", fontSize: "2rem", fontWeight: "bold", textAlign:"left" }}>{room.roomName}</p>
